...
export function useMapPins(mapInstance: any) {
  ...
  const saveCameraPin = async ({
    ...
    try {
      ...
      cameraMarker.style.position = 'relative';
      cameraMarker.style.width = '48px';
      cameraMarker.style.height = '48px';
      cameraMarker.style.display = 'flex';
      cameraMarker.style.justifyContent = 'center';
      cameraMarker.style.alignItems = 'center';
      cameraMarker.style.background = 'transparent';
      cameraMarker.style.filter = 'drop-shadow(0 0 4px rgba(0,0,0,0.5))';
      ...
      cone.style.borderBottom = '80px solid rgba(0, 136, 255, 0.3)'; // transparent blue cone
