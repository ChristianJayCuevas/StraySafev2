...
function getActiveTheme(appearance: 'light' | 'dark' | 'system') {
...
async function initializeMap() {
  ...
  try {
    ...
    const theme = getActiveTheme(appearance.value)
    const mapStyle =
      theme === 'dark'
...
<template>
  ...
  <div class="relative">
   ...
   <!--<Button @click="simulateStrayDog" class="absolute top-4 right-4 z-10 bg-blue-500 text-white">
    ...
    <!-- Current Map Label - now positioned relative to the map container -->
  ...
  <div class="flex flex-col gap-2">
    ...
    <!-- NEW: Orange animal heatmap -->
    <div class="flex items-center gap-2">
      <div class="w-4 h-4 rounded-sm bg-gradient-to-r from-orange-200 to-orange-800"></div>
      <span>Animal Density</span>
    </div>
